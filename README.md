...
# 1) 准备环境变量
cp .env.example .env
# 编辑 .env：设 YEVUNE_APP_SECRET（如 `openssl rand -hex 32`）
...
#    建 bucket 并签发密钥：
docker compose exec garage /garage bucket create yevune
docker compose exec garage /garage key create yevune-key
#    把上一步输出的 Key ID / Secret 写入 .env 的 GARAGE_ACCESS_KEY / GARAGE_SECRET_KEY，并授权：
docker compose exec garage /garage bucket allow --read --write yevune --key yevune-key

# 4) 起服务端（首次会构建 musl 静态镜像）
docker compose up -d yevune

# 5) 查看首启管理员密码（未在 .env 指定 YEVUNE_ADMIN_PASSWORD 时自动生成）
docker compose logs yevune | grep 首启
```
...
把音频文件放进 Garage 的 `yevune` bucket（S3 客户端上传，或用扩展接口 `uploadTrack`），
然后触发扫描：`curl "http://localhost:4533/rest/startScan?u=admin&p=<密码>&v=1.16.1&c=app"`。

## 配置

单一 TOML（`config.toml`）+ 环境变量覆盖（前缀 `YEVUNE`，分隔符 `__`，如
`YEVUNE__SERVER__PORT=8080`）。全部字段带合理默认，详见 [`server/src/config.rs`](server/src/config.rs)。
密钥 `YEVUNE_APP_SECRET` 用于密码加密与会话签名，必须设置。
