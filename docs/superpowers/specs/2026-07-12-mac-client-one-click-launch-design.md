...
- 比较 core 源码、Cargo 清单、UniFFI 配置与现有 xcframework 的修改时间；产物缺失或输入更新时执行 `clients/apple/Packages/YevuneCoreFFI/scripts/build-core.sh`。
- 随后执行 `swift run --package-path clients/apple Yevune`，并用 `exec` 将信号与退出码直接交给客户端进程。
