...
同时把顶部 `use yevune_core::{AlbumSort, MusicClient};` 改为 `use yevune_core::{AlbumFilter, AlbumSort, MusicClient};`，并把原有测试里的 `client.list_albums(AlbumSort::Newest, 0, 50)` 改成 `client.list_albums(AlbumFilter::Sort(AlbumSort::Newest), 0, 50)`。
...
**Files:**
- No source changes; runs `clients/apple/Packages/YevuneCoreFFI/scripts/build-core.sh`.

**Interfaces:**
- Consumes: Task 1/2 产出的 `AlbumFilter`、`list_genres`、`Genre`。
- Produces: 重新生成的 `clients/apple/Packages/YevuneCoreFFI/Sources/YevuneCoreFFI/YevuneCoreFFI.swift`，供 Task 4 使用（`AlbumFilter` 枚举、`Genre` 记录、`MusicClient.listAlbums(filter:offset:size:)`、`MusicClient.listGenres()`）。
...
Expected: 脚本成功结束，生成 `clients/apple/Packages/YevuneCoreFFI/Sources/YevuneCoreFFI/YevuneCoreFFI.swift`

- [ ] **Step 3: 验证生成文件包含新符号**

Run: `grep -n "enum AlbumFilter\|struct Genre\|func listGenres\|func listAlbums" clients/apple/Packages/YevuneCoreFFI/Sources/YevuneCoreFFI/YevuneCoreFFI.swift`
Expected: 输出包含 `AlbumFilter`、`Genre`、`listGenres`、两个 `listAlbums` 重载

- [ ] **Step 4: 提交生成产物**

```bash
git add clients/apple/Packages/YevuneCoreFFI/Sources/YevuneCoreFFI clients/apple/Packages/YevuneCoreFFI/YevuneCoreFFI.xcframework
...
**Files:**
- Modify: `clients/apple/Sources/Yevune/Model/LoginViewModel.swift`（仅 `MusicClientProviding` 协议与扩展部分）
- Modify: `clients/apple/Sources/Yevune/Model/CoreMusicClient.swift`

**Interfaces:**
- Consumes: `YevuneCoreFFI.AlbumFilter`、`YevuneCoreFFI.Genre`、`YevuneCoreFFI.MusicClient.listAlbums(filter:offset:size:)`、`YevuneCoreFFI.MusicClient.listGenres()`（Task 3 产出）。
...
```bash
git add clients/apple/Sources/Yevune/Model/LoginViewModel.swift clients/apple/Sources/Yevune/Model/CoreMusicClient.swift
...
- Test: `clients/apple/Tests/YevuneTests/LibraryViewModelTests.swift`（新建）
...
创建 `clients/apple/Tests/YevuneTests/LibraryViewModelTests.swift`：
...
```bash
git add clients/apple/Sources/Yevune/Model/LibraryViewModel.swift clients/apple/Tests/YevuneTests/LibraryViewModelTests.swift
...
**Files:**
- Create: `clients/apple/Sources/Yevune/Views/AlbumGridView.swift`
- Modify: `clients/apple/Sources/Yevune/Views/LibraryView.swift`（仅 `libraryDetail` 及相关 `@State`/`.task`）
...
Run: `cd clients/apple && swift run Yevune`（或通过 Xcode 运行），登录后在曲库区依次：切换排序 Picker、选流派、勾选年份区间、切换网格/列表，确认专辑列表随之刷新且网格封面可点击进入详情。

- [ ] **Step 6: 提交**

```bash
git add clients/apple/Sources/Yevune/Views/AlbumGridView.swift clients/apple/Sources/Yevune/Views/LibraryView.swift
