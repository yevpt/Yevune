...
- `clients/apple/Sources/Yevune/Model/LoginViewModel.swift`（改）：`MusicClientProviding` 增歌单方法 + 默认实现扩展。
- `clients/apple/Sources/Yevune/Model/CoreMusicClient.swift`（改）：桥接歌单方法到 YevuneCoreFFI。
- `clients/apple/Sources/Yevune/Model/PlaylistViewModel.swift`（建）：树/详情状态 + CRUD。
- `clients/apple/Sources/Yevune/Views/PlaylistDetailView.swift`（建）：歌单曲目详情。
- `clients/apple/Sources/Yevune/Views/LibraryView.swift`（改）：侧栏分区 + `SidebarSelection` + 菜单接线。
- `clients/apple/Sources/Yevune/Views/MediaDetailView.swift`（改）：曲目行「加入歌单」子菜单。
- `clients/apple/Tests/YevuneTests/PlaylistViewModelTests.swift`（建）：ViewModel XCTest + `FakePlaylistClient`。
...
Run: `cargo test -p yevune-core --test playlist_test playlist_tree_decodes_folders_and_playlists`
...
Run: `cargo test -p yevune-core --test playlist_test`
Expected: PASS。

- [ ] **Step 6: 提交**

```bash
cargo fmt && cargo clippy -p yevune-core -- -D warnings
...
Run: `cargo test -p yevune-core --test playlist_test playlist_detail_decodes_playlist_and_entries`
...
Run: `cargo test -p yevune-core --test playlist_test create_playlist_without_folder_sends_single_request`
Expected: 编译失败（方法未定义）。
...
Run: `cargo test -p yevune-core --test playlist_test rename_and_add_and_remove_encode_params`
Expected: 编译失败。
...
Run: `cargo test -p yevune-core --test playlist_test create_folder_decodes_and_move_encodes`
Expected: 编译失败。
...
Run: `cargo test -p yevune-core && cargo clippy -p yevune-core -- -D warnings && cargo fmt --check`
Expected: 全 PASS。
...
**Interfaces:**
- Consumes: YevuneCoreFFI 生成类型 `PlaylistTree`、`PlaylistDetail`、`Playlist`、`PlaylistFolder`（来自 Task 1–5）。
- Produces: `MusicClientProviding` 新增歌单方法（带 throwing 默认实现，供既有 Fake 免改）；`CoreMusicClient` 实现转发。

- [ ] **Step 1: 重建 YevuneCoreFFI 绑定**
...
然后执行其中的 core 构建 / uniffi-bindgen 段，使 `clients/apple/Packages/YevuneCoreFFI/Sources/YevuneCoreFFI/YevuneCoreFFI.swift` 含 `playlistTree`、`playlistDetail`、`createPlaylist` 等新方法。

- [ ] **Step 2: 校验绑定含新方法**

Run: `grep -n "func playlistTree\|func playlistDetail\|func createPlaylist\|func createFolder" clients/apple/Packages/YevuneCoreFFI/Sources/YevuneCoreFFI/YevuneCoreFFI.swift`
...
```bash
git add clients/apple/Packages/YevuneCoreFFI clients/apple/Sources/Yevune/Model/LoginViewModel.swift clients/apple/Sources/Yevune/Model/CoreMusicClient.swift
...
**Files:**
- Create: `clients/apple/Sources/Yevune/Model/PlaylistViewModel.swift`
- Create: `clients/apple/Tests/YevuneTests/PlaylistViewModelTests.swift`
...
`clients/apple/Tests/YevuneTests/PlaylistViewModelTests.swift`：
...
`clients/apple/Sources/Yevune/Model/PlaylistViewModel.swift`：
...
```bash
git add clients/apple/Sources/Yevune/Model/PlaylistViewModel.swift clients/apple/Tests/YevuneTests/PlaylistViewModelTests.swift
...
**Files:**
- Modify: `clients/apple/Sources/Yevune/Model/PlaylistViewModel.swift`
- Modify: `clients/apple/Tests/YevuneTests/PlaylistViewModelTests.swift`
...
**Files:**
- Create: `clients/apple/Sources/Yevune/Views/PlaylistDetailView.swift`
...
```bash
git add clients/apple/Sources/Yevune/Views/PlaylistDetailView.swift clients/apple/Sources/Yevune/Views/LibraryView.swift
...
```bash
git add clients/apple/Sources/Yevune/Views/MediaDetailView.swift clients/apple/Sources/Yevune/Views/LibraryView.swift
