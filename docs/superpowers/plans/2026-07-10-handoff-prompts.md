...
```
你在为一个自托管家庭音乐服务的 Rust 服务端工作。仓库根有 AGENTS.md（项目宪法，含架构红线与强制工作流），docs/superpowers/specs/2026-07-10-yevune-server-design.md（设计），docs/superpowers/plans/2026-07-10-yevune-server.md（本任务详情 T0）。开始前必须读完这三份。
...
```
你在为一个自托管家庭音乐服务的 Rust 服务端工作。仓库根有 AGENTS.md（项目宪法），docs/superpowers/specs/2026-07-10-yevune-server-design.md（设计，重点看 §6/§9），docs/superpowers/plans/2026-07-10-yevune-server.md（本任务 T1）。开始前必须读完。
...
```
你在为一个自托管家庭音乐服务的 Rust 服务端工作。仓库根有 AGENTS.md（宪法，注意 SQLite 放本地磁盘、禁止 Postgres/Redis 的红线），docs/superpowers/specs/2026-07-10-yevune-server-design.md（设计 §6），docs/superpowers/plans/2026-07-10-yevune-server.md（本任务 T2）。开始前必须读完。
...
```
你在为一个自托管家庭音乐服务的 Rust 服务端工作。仓库根有 AGENTS.md（宪法），docs/superpowers/specs/2026-07-10-yevune-server-design.md（设计），docs/superpowers/plans/2026-07-10-yevune-server.md（本任务 T3）。开始前必须读完。
...
```
你在为一个自托管家庭音乐服务的 Rust 服务端工作。仓库根有 AGENTS.md（宪法），docs/superpowers/specs/2026-07-10-yevune-server-design.md（设计 §10），docs/superpowers/plans/2026-07-10-yevune-server.md（本任务 T6）。开始前必须读完。
...
```
你在为一个自托管家庭音乐服务的 Rust 服务端工作。仓库根有 AGENTS.md（宪法，注意"绝不把整文件读进内存"红线），docs/superpowers/specs/2026-07-10-yevune-server-design.md（设计 §7），docs/superpowers/plans/2026-07-10-yevune-server.md（本任务 T4）。开始前必须读完。
...
```
你在为一个自托管家庭音乐服务的 Rust 服务端工作。仓库根有 AGENTS.md（宪法，注意"绝不缓存不完整转码产物""流式有界缓冲"红线），docs/superpowers/specs/2026-07-10-yevune-server-design.md（设计 §8），docs/superpowers/plans/2026-07-10-yevune-server.md（本任务 T5）。开始前必须读完。
...
```
你在为一个自托管家庭音乐服务的 Rust 服务端工作。仓库根有 AGENTS.md（宪法，注意"禁止破坏 OpenSubsonic 兼容"红线），docs/superpowers/specs/2026-07-10-yevune-server-design.md（设计 §9），docs/superpowers/plans/2026-07-10-yevune-server.md（本任务 T7）。开始前必须读完。
...
```
你在为一个自托管家庭音乐服务的 Rust 服务端工作。仓库根有 AGENTS.md（宪法，扩展走 /rest/ext/* 并在 getOpenSubsonicExtensions 声明），docs/superpowers/specs/2026-07-10-yevune-server-design.md（设计 §9），docs/superpowers/plans/2026-07-10-yevune-server.md（本任务 T8）。开始前必须读完。
...
```
你在为一个自托管家庭音乐服务的 Rust 服务端工作。仓库根有 AGENTS.md（宪法，"授权服务端强制、客户端不可绕过"红线），docs/superpowers/specs/2026-07-10-yevune-server-design.md（设计 §6 曲库访问控制模型），docs/superpowers/plans/2026-07-10-yevune-server.md（本任务 T9）。开始前必须读完。
...
```
你在为一个自托管家庭音乐服务的 Rust 服务端工作。仓库根有 AGENTS.md（宪法），docs/superpowers/specs/2026-07-10-yevune-server-design.md（设计 §11），docs/superpowers/plans/2026-07-10-yevune-server.md（本任务 T10）。开始前必须读完。
