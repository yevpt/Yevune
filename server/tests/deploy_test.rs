...
//! yevune+garage 两服务、FFmpeg 打包进镜像、musl 静态构建、首启密钥与建管理员就位。
...
fn compose_注入必需环境变量() {
    ...
    for key in [
        "YEVUNE_APP_SECRET",
        "YEVUNE__GARAGE__ENDPOINT",
        "YEVUNE__DATABASE__PATH",
        "YEVUNE__SETUP__ADMIN_USERNAME",
    ] {
        assert!(compose.contains(key), "compose 缺环境变量 {key}");
    }
    assert!(
        compose.contains("YEVUNE__GARAGE__BUCKET: \"yevune\""),
        "compose 应将 YEVUNE__GARAGE__BUCKET 设为 yevune"
    );
}
...
fn dockerfile_为_musl_静态构建且附带_ffmpeg() {
    ...
    assert!(
        dockerfile.contains("--bin yevune-server"),
        "应构建 yevune-server 二进制"
    );
    assert!(
