...
//!
//! 用真实 HTTP 请求驱动 [`music_server::app_with_state`]，断言受限内容对无授权用户
...
async fn get_song_受限曲目对无授权者遮蔽对授权者与管理员可见() {
    ...
    let uri = format!("/rest/getSong?id={}&f=json", f.secret_track);
    ...
    assert_eq!(
        alice["subsonic-response"]["song"]["id"],
        f.secret_track.to_string()
    );

    let (_, root) = ctx.get_json(&uri, Some(&ctx.bearer(f.root))).await;
    assert_eq!(
        root["subsonic-response"]["song"]["id"],
        f.secret_track.to_string()
    );
}
...
async fn get_album_歌曲列表按可见性过滤() {
    let ctx = common::ctx().await;
    let f = seed(&ctx).await;
    let uri = format!("/rest/getAlbum?id={}&f=json", f.album1);

    let (_, bob) = ctx.get_json(&uri, Some(&ctx.bearer(f.bob))).await;
    let bob_songs = ids(&bob["subsonic-response"]["album"]["song"]);
    assert_eq!(
        bob_songs,
        vec![f.open_track.to_string()],
        "bob 只见公开曲目"
    );
    ...
    alice_songs.sort();
    let mut want = vec![f.open_track.to_string(), f.secret_track.to_string()];
    want.sort();
...
async fn get_artists_隐藏无可见曲目的艺人() {
    ...
    assert!(
        bob_ids.contains(&f.artist1.to_string()),
        "有公开曲目的艺人可见"
    );
    assert!(
        !bob_ids.contains(&f.artist2.to_string()),
        "全受限艺人对 bob 不可见"
    );
    ...
    assert!(
        alice_ids.contains(&f.artist2.to_string()),
        "alice 见受限艺人"
    );
}
...
async fn get_indexes_与_get_artists_同样过滤() {
    ...
    assert!(!bob_ids.contains(&f.artist2.to_string()));
}
...
async fn get_album_list2_隐藏无可见曲目的专辑() {
    ...
    let (_, bob) = ctx
        .get_json(
            "/rest/getAlbumList2?f=json&size=50",
            Some(&ctx.bearer(f.bob)),
        )
        .await;
    let bob_albums = ids(&bob["subsonic-response"]["albumList2"]["album"]);
    assert!(bob_albums.contains(&f.album1.to_string()));
    assert!(
        !bob_albums.contains(&f.album2.to_string()),
        "隐藏专辑不出现"
    );
    ...
    assert!(alice_albums.contains(&f.album2.to_string()));
}
...
async fn 未认证请求被拒() {
    ...
    let f = seed(&ctx).await;
    let (status, _) = ctx
        .get(&format!("/rest/getSong?id={}&f=json", f.open_track), None)
        .await;
...
async fn get_album_默认返回_xml() {
    ...
    let (status, body) = ctx
        .get(
            &format!("/rest/getAlbum?id={}", f.album1),
            Some(&ctx.bearer(f.alice)),
        )
        .await;
